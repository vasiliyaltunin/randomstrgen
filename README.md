# RandomStrGen библиотека для создания случайных строк
...
### Возможности
Создание случайной строки заданной длины с возможностью исключения символов из процесса генерации.
...
- В **главный** файл CMakeLists.txt добавьте :
  ...
  add_subdirectory(lib/randomstrgen)
  ```
...
```cmake
target_link_libraries(${PROJECT_NAME} PRIVATE altuninvv::randomstrgen)
```
...
```cpp
#include <randomstrgen/randomstrgen.h>
```
...
FetchContent_Declare(
  randomstrgen  # Имя target нашей библиотеки, должно совпадать с используемым далее 
  GIT_REPOSITORY https://github.com/vasiliyaltunin/randomstrgen.git # Путь до репозитория
...
if(NOT randomstrgen_POPULATED)
    FetchContent_MakeAvailable(randomstrgen)
endif()

target_include_directories(${PROJECT_NAME} PRIVATE ${randomstrgen_SOURCE_DIR}/include)

target_link_libraries(${PROJECT_NAME}
    PRIVATE  
    altuninvv::randomstrgen
)
